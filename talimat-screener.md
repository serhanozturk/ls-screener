...
- v15: Backtest optimizasyonu - hacim tabani 100K->250K, ayrisma<-15 "zayif" etiketi
- v16: PATLAMA guc kademesi (3 seviye: GUCLU/NORMAL/ZAYIF, Supabase strength kolonu)
...
- GUC KADEMESI (v16, 3 seviye - _pump_strength fonksiyonu, ELENMEZ sadece etiket):
  * GUCLU (dolar-banknot ikonu): erken uyari teyidi VAR + ayrisma > 0 + skor 3/3.
    Backtest: bu kombinasyon tepe>=20% %42 (genel %30). BANK/AKE profili =
    once 15m erken uyari, sonra patlama, whale long, funding negatif.
  * ZAYIF (beyaz top ikonu): ayrisma < PUMP_WEAK_DIV (-15), whale katilmiyor. Backtest %17.
  * NORMAL (sari top ikonu): gerisi.
  Cakisma imkansiz (skor3 -> whale long -> ayrisma>0 -> asla <-15 olamaz).
  Telegram: coin adi altinda AYRI SATIR. Dashboard: badge ikonu. Supabase: strength kolonu
  (strong/weak/normal; ALTER TABLE ile eklenir - yoksa INSERT 400 verir).
  Onemli: "skor 3 tek basina" degil, teyit ZORUNLU (backtest'te grup zaten teyitliydi).
  Funding'i cikarmak (skor 2) isabeti dusuruyor - funding<=0 ayirici faktor.
...
3. Mum hacmi >= 250K USDT (EARLY_MIN_VOL, olu coin filtresi; v15: 100K->250K)
...
BACKTEST BULGULARI (v15, 8 gun / 1917 erken uyari):
- Erken uyarilarin %12'si 6h icinde PATLAMA teyidi aldi. Teyitli grup tepe>=20% %30,
  teyitsiz %15 - teyit isabeti ~2x artiriyor ama teyitsizlerin de %15'i +20% yapiyor.
- Mutlak hacim AYIRT EDICI: >=250K taban isabeti %16->%22, >=500K %24, >=1M %25.
  Iyi yukselen hacim medyani 424K, kotu 235K.
- Carpan (vol_x/tb_x) yukseltmek ISE YARAMIYOR: iyi/kotu medyanlari ayni (8.2 vs 8.9),
  20x'te isabet dusuyor. Carpan "ani mi" der, "buyuk mu" demez - o yuzden 5x sabit kaldi.
- OI<->yukselis korelasyonu ~0 (0.009). OI KAPISI ise yariyor (gecen %30 vs gecemeyen %15)
  ama kapi SONRASI OI% derecesi onemsiz - o yuzden %10 esigi ve derecelendirme degismedi.
- Ayrisma AYIRT EDICI: iyi yukselen medyan -0.47, kotu -2.36. Ayrisma>=0 grubu tepe>=20% %34,
  ayrisma<-15 grubu %17. Cok negatif ayrisma = retail FOMO, whale yok = zayif hareket.
...
- RLS disabled (Engine kalibi). v16: strength kolonu eklendi
  (ALTER TABLE screener_signals ADD COLUMN IF NOT EXISTS strength text).
  Kolon yoksa PUMP INSERT'leri 400 verir. Sonuc olcumu kayit aninda YAPILMAZ —
...
- Deploy sonrasi loglarda "v16 listening" gor = dogrulama
- Erisim: http://178.104.143.245:8766
- Env listesi (Coolify): TELEGRAM_TOKEN, TELEGRAM_CHAT_IDS, SUPABASE_URL, SUPABASE_KEY

## BACKTEST DISIPLINI (her backtest/analiz oncesi uygula)
Bir esik/kural/bulguyu KABUL ETMEDEN once bu listeyi gec. Amac: "veri bir sey
soyluyor gibi gorundu ama gurultuydu" tuzagina dusmemek. Bu liste bir kez gercek
bir yanilsamayi yakaladi (asagida ornek).
1. BULGUYU CURUT: Esigi DESTEKLEYEN degil CURUTEN testi calistir. "Aralik ICI,
   DISINDAN belirgin ustun mu?" kafa kafaya kiyas. Ornek tuzak: "-5..5 ayrisma
   tatli nokta" tepe>=50% metriginde iyi gorundu, ama median + tepe>=20% ile
   disiyla kiyaslaninca ustunluk YOK cikti = yanilsama.
2. TEK METRIGE GUVENME: En az 3 metrik (median tepe, tepe>=20%, tepe>=50%,
   guncel% = kalici mi geri mi verdi). Metrikler celisirse bulgu zayif.
3. ORNEKLEM + GUVEN ARALIGI: Her grup icin n kac, %95 Wilson araligi ne genis?
   Araliklar CAKISIYORSA "fark var" DEME. n<10'dan kalici kural CIKARMA.
4. EDGE-CASE AYRISTIR: Bir "ele/filtrele" kurali onermeden once eleyecegi grupta
   TERS senaryo var mi bak. Ornek: "OI<0 ise ele" dersen OI<0 + funding cok
   negatif (short squeeze) adaylarini da elersin - onlar %41 isabet veriyordu.
5. KOR NOKTA BEYAN ET: Test EDEMEDIGIN seyi acikca soyle, VARSAYMA. Ornek:
   PATLAMA kayitlarinda mutlak hacim yok -> likidite segmentasyonu YAPILAMIYOR.
6. ORNEKLEM/DONEM UYARISI: Kac gun, kac sinyal, kac piyasa donemi? Tek donem +
   kucuk orneklem = kalici kural icin YETERSIZ. "Yon gosterdi, daha fazla veri
   gerek" de, kesin karar verme.
KURAL: Bu listeyi gecemeyen bulgu KALICI kurala GIRMEZ. Ilginc bulgular
"izlenecek hipotez" olarak not edilir, veri birikince tekrar test edilir.

## BEKLEYEN / ACIK KONULAR
- AYRISMA DONUS SINYALI (fikir): coin ayrismasi negatiften pozitife donunce "whale katildi"
  uyarisi. AKE 3 Tem -20.3 zayif basladi, 15 Tem +1.7'ye donunce asil yukselis (~16x).
  Ayrisma zaten hesaplaniyor, onceki degeri bellekte tutup donusu yakalamak - sifir ekstra istek.
  Yeni veriyle dogrulanacak.
